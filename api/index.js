...
function getPool() {
  if (!pool) {
    if (!process.env.DATABASE_URL) {
      throw new Error("DATABASE_URL is not set. Add it in your Vercel environment variables.");
    }
...
app.get("/api/cars", async (_req, res) => {
  try {
    const p = getPool();
    const { rows } = await p.query("SELECT * FROM cars ORDER BY sort_order ASC");
    res.json(rows.map(rowToApi));
  } catch (err) {
    res.status(500).json({ error: "Failed to fetch cars", detail: err.message });
  }
});
...
app.delete("/api/cars/:id", requireAdmin, async (req, res) => {
  try {
    const p = getPool();
    await p.query("DELETE FROM cars WHERE id=$1", [parseInt(req.params.id)]);
    res.json({ ok: true });
  } catch (err) {
    res.status(500).json({ error: "Failed to delete car", detail: err.message });
  }
});
...
app.patch("/api/cars/:id/toggle", requireAdmin, async (req, res) => {
  try {
    const p = getPool();
    const id = parseInt(req.params.id);
    const { rows: [car] } = await p.query("SELECT status FROM cars WHERE id=$1", [id]);
    if (!car) { res.status(404).json({ error: "Car not found" }); return; }
    const newStatus = car.status === "available" ? "sold" : "available";
    const { rows: [updated] } = await p.query("UPDATE cars SET status=$1 WHERE id=$2 RETURNING *", [newStatus, id]);
    res.json(rowToApi(updated));
  } catch (err) {
    res.status(500).json({ error: "Failed to toggle status", detail: err.message });
  }
});
...
app.patch("/api/cars/:id/move", requireAdmin, async (req, res) => {
  try {
    ...
    const { rows: all } = await p.query("SELECT id, sort_order FROM cars ORDER BY sort_order ASC");
    const idx = all.findIndex(c => c.id === id);
    if (idx < 0) { res.status(404).json({ error: "Not found" }); return; }
    const swapIdx = direction === "up" ? idx - 1 : idx + 1;
    if (swapIdx < 0 || swapIdx >= all.length) { res.json({ ok: true }); return; }
    const a = all[idx], b = all[swapIdx];
    await p.query("UPDATE cars SET sort_order=$1 WHERE id=$2", [b.sort_order, a.id]);
    await p.query("UPDATE cars SET sort_order=$1 WHERE id=$2", [a.sort_order, b.id]);
    res.json({ ok: true });
  } catch (err) {
    res.status(500).json({ error: "Failed to move car", detail: err.message });
  }
});
